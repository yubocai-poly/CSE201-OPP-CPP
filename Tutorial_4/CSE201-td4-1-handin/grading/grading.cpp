...
bool test_simulate_step_aux(std::ostream &out,
  ...
  // allow both the correct and the off by one solutions (the wrong off by one solution where
  // we update velocity_y before updating the position was used as
  // reference in the test case)
  success = (abs(x - next_x) <= 0.0001 && (abs(y - next_y) <= 0.0001 ||
                                           abs(y - next_y_wrong) <= 0.0001));
...
#if EXERCISE_4 == 1
      //
      test_simulate_step_aux(out, 0, 0, 0, 0, 0.05, 0, -0.01225, -0.03675),
      test_simulate_step_aux(out, 5, 6, 45, 70, 0.001, 5.0154, 6.0423, 6.0423),
      test_simulate_step_aux(out, 3, 3, 45, 70, 0.001, 3.0154, 3.0423, 3.0423),
      test_simulate_step_aux(out, 10, 10, 50, 20, 0.001, 10.047, 10.017, 10.017)
#endif
    };
...
void fill_telemetry(Telemetry &telemetry,
                    ...
                    const bool wrong_computation) { // hack to take into account the off-by one error
  double x, y, t, vx, vy;
  bool hit_target;
  double g = -9.8;
    ...
    auto get0 = [](std::ostream &out, bool (*compare_x)(std::ostream&,std::string,double,double,double,double,double,double))
      {
        ...
        return compare_x(out,
                         "Telemetry t; t.add_point(0,0,0); t.add_point(1,2,3); t.get_point(0, time,x,y);",
                         time, x, y,
                         0,0,0);
      };
    ...
    auto get1 = [](std::ostream &out, bool (*compare_x)(std::ostream&,std::string,double,double,double,double,double,double))
      {
        ...
        return compare_x(out,
                         "Telemetry t; t.add_point(0,0,0); t.add_point(1,2,3); t.get_point(1, time,x,y);",
                         time, x, y,
                         1,2,3);
      };
#endif
...
int test_game(std::ostream &out, const std::string test_name) {
    ...
    auto test_game_aux = [](std::ostream &out,
                            ...
                            double xt, double yt)
      {
        ...
        double diff =  (abs( (double) (res_count - exp_count))) * 100 / (res_count + exp_count);
        ...
        if (! fail_count) {
          ...
          i = 0;
          j = 0;
          for (; i < res_count && j < exp_count; ++i, ++j) {
            double t,x,y,t1,x1,y1;
            double tolerance = 0.01;

            auto compare_robust = [](double a, double b, double tolerance) {
              return (abs(a - b)) <= tolerance;
            };

            g.telemetry.get_point(i,t,x,y);
            expected_telemetry.get_point(j,t1,x1,y1);

            if ((t != t1) ||                              // time should be the same, compared in order
                (!compare_robust(x,x1, i * tolerance)) || // relative accumulation error
                (!compare_robust(y,y1, j * tolerance))) { // relative accumulation error
        ...
        out << "Target(Coordinate(" << xt << "," << yt << ")" << ",1.0); ";
        out << "Game g(p, t); game.run(" << simulation_interval << ");";
        out << ": got ";

        if (fail_count || success) {
        ...
        if (success) {
          // better reporting, tell that we expected the same amount of elements returned (with 5% tolerance)
          out << res_count << " elements";
        } else if (fail_count) {
