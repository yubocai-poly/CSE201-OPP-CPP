...
//  double second_var = numeric_limits<double>::max();
  double first_var = 0; // 0 is more user-friendly for students than the double limit
  double second_var = 0;
...
// TODO: change it to a generic array (not an array with couple of doubles, like the targets
template <typename T>
...
bool test_sorted(std::ostream &out,
    ...
    for(int i = 0; success && i < tot_elements; ++i) {
      bool found_copy = false;
      for(int j = 0; !found_copy && j < tot_elements; ++j) {
        if (copy[2*j] == to_sort[2*i] &&
            copy[2*j + 1] == to_sort[2*i + 1])
          found_copy = true;
      }
    ...
    if (success) {
      ...
      if (wrong_order) 
        out << ": got " << o1 << " > " << o2 << " expected a sorted array";
      else
        out << ": cannot find the pair (" << o1 << ", " << o2 << ") in the sorted array";
    }
    out << std::endl;
