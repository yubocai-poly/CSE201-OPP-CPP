...
int main(int argc, char* argv[])
{
    ...
    {
      simulate_full_trajectory(std::cerr, simulation_interval, p1_reference); // this function takes a DroppingProjectile as input - so no problem here
      simulate_full_trajectory(std::cerr, simulation_interval, &p1);
      ...
      std::cout << "Expected result: ("
                << p1_reference.get_position().get_x() << ","
                << p1_reference.get_position().get_y() << ")" << std::endl;

      std::cout << "Position of p1: ("
                << p1.get_position().get_x() << ","
                << p1.get_position().get_y() << ")\n" << std::endl;
    }
#endif
