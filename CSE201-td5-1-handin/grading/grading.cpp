  ...
  auto test_projectile = [](ostream& out,
    ...
    while (dropping.get_position().get_y() >= 0) {
      if (reference.get_velocity_y() > 0) {
        // assert(reference.get_position().get_x() == dropping.get_position().get_x());
        // assert(reference.get_position().get_y() == dropping.get_position().get_y());
    ...
    return (
            test_eq_approx(out,
                           ...
                           dropping.get_position().get_x(),
                           last_x,
                           TOLERANCE) +
            test_eq_approx(out,
                           description + " dropping.et_position().get_y",
                           dropping.get_position().get_y(),
                           reference.get_position().get_y(),
                           TOLERANCE) +
            // [SM] 6/10/2021: drop the max coordinate
            // test_eq(out,
...
int projectile_simulation(std::ostream &out, const std::string test_name) {
  ...
  auto test_projectile = [](ostream& out,
    ...
    while (ptr_proj_1->get_position().get_y() >= 0) {
      if (reference.get_velocity_y() > 0) {
        // assert(reference.get_position().get_x() == ptr_proj_1->get_position().get_x());
        // assert(reference.get_position().get_y() == ptr_proj_1->get_position().get_y());
